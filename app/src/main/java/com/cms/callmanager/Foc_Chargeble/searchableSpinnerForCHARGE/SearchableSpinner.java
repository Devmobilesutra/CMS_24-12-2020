   ...
   public static String item_values;
    ...
    public SearchableSpinner(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            Log.d("", "SearchableSpinner_getIndex: "+ a.getIndex(i));
            ...
            if (null != _arrayAdapter) {
              ...
              /*  _items.clear();
                for (int i = 0; i < _arrayAdapter.getCount(); i++) {
                    ...
                    Log.d("", "onTouch_add: "+_items);
                }

                for (ModelClassForSavedData item: FOC_ListAdapter.list_foc) {

                    _items.remove(item.getSelectedPosition());
                    Log.d("", "onTouch_remove: "+item.getSelectedPosition());*/
                _items.clear();
                for (int i = 0; i < _arrayAdapter.getCount(); i++) {
                    _items.add(_arrayAdapter.getItem(i));

                 //   Log.d("", "onTouch_adddddd: "+_arrayAdapter.getItem(i)+""+    FOC_ListAdapter.poss);
/*
                    if (_arrayAdapter.getItem(i)== FOC_ListAdapter.poss){

                      //  Toast.makeText(_context, "SAME ELEMENT", Toast.LENGTH_SHORT).show();

                        _items.remove(_arrayAdapter.getItem(i));
                    }*/
                }
    ...
    public void onSearchableItemClicked(Object item, int position) {
        setSelection(_items.indexOf(item));
         ...
         item_values = (String) item;

        Log.d("", "onSearchableItemClicked_item: "+item_values);
        if (!_isDirty) {
            _isDirty = true;
